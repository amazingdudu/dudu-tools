...
const { getProjectPath, getConfig } = require('./utils/projectUtils');
const { cssInjection } = require('./utils/styleUtil');

const stylesSrc = [
    getProjectPath('src/components/**/*.less'),
    `!${getProjectPath('src/components/**/demo/**')}`
];
...
const scriptsSrc = [
    ...
    `!${getProjectPath('src/components/**/demo/**')}`,
    `!${getProjectPath('src/components/**/__test__/**')}`
];
...
function compileStyles(dir) {
    return function compileStyles() {
        const plugins = getConfig().postcss || [];
        return gulp
            .src(stylesSrc)
